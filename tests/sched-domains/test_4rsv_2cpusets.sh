...
if [ $? -eq 0 ]; then
  test_fail "cpusetB cpumask changed"
  trace_write "Sleep for 1s"
  sleep 1
  tear_down
  exit 1
...
if [ $? -ne 0 ]; then
  test_fail "couldn't modify $PID4 to (1,20)"
  tear_down
  exit 1
fi
...
if [ $? -eq 0 ]; then
  ...
  test_fail "couldn't remove CPU 1"
  trace_write "Sleep for 1s"
  sleep 1
  tear_down
  exit 1
...
if [ $? -eq 0 ]; then
  ...
  test_fail "couldn't attach CPU 1"
  trace_write "Sleep for 1s"
  sleep 1
  tear_down
  exit 1
...
if [ $? -eq 0 ]; then
  ...
  test_fail "cpusetA couldn't be removed"
  trace_write "Sleep for 1s"
  sleep 1
  tear_down_nokill
  exit 1
...
if [ $? -eq 0 ]; then
  ...
  test_fail "cpusetB couldn't be removed"
  trace_write "Sleep for 1s"
  sleep 1
  tear_down_nokill
  exit 1
